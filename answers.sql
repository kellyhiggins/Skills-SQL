...
SELECT models.name, models.brand_name, brands.headquarters FROM models JOIN brands ON models.brand_name=brands.name WHERE models.name = 'Mustang';
...
SELECT models.name, models.brand_name, brands.founded FROM models LEFT JOIN brands ON (models.brand_name=brands.name) WHERE year = '1960'; 
...
SELECT b.name, founded FROM brands AS b LEFT JOIN models AS m ON (b.name = m.brand_name) WHERE m.id IS NULL;
...
SELECT m.brand_name FROM brands AS b JOIN models as m ON (b.name = m.brand_name) GROUP BY m.brand_name HAVING COUNT(*) > 5;
...
SELECT m.name, m.year FROM models AS m WHERE m.year IN (SELECT DISTINCT founded FROM brands);